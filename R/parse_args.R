...
#' \itemize{
#'   \item df.name The data frame name when applicable.
#'   \item var.names The variable names when applicable.
#'   \item by.group The group, when functions are called through
#'     \code{by()}
#'   \item by.first A binary indicator used when function was called
#'     through \code{by()}
#'   \item by.last A binary indicator used when function was called
#'     through \code{by()}}
#'
...
parse_args <- function(sys_calls, sys_frames, match_call, 
  ...
  if (length(with_pos) == 1) {
    ...
    if (length(by_pos) == 0) {
      ...
      if (is.data.frame(tmp_)) {
        df_ <- tmp_
        df.name <- deparse(with_call$data)
        allnames <- all.vars(with_call$expr)
        if (length(allnames) > 0) {
          if (var == "x") {
            var.names <- intersect(allnames, with_objects)[1]
          } else {
            var.names <- intersect(allnames, with_objects)[2]
          }
        } 
      ...
      if (is.data.frame(eval(with_call$data))) {
        df.name <- deparse(with_call$data)
        var.names <- deparse(by_call$data)
        ...
        if (length(with_objects) == 1 &&
          ...
          if (length(all.vars(by_call$data)) == 1) {
            var.names <- colnames(eval(with_call$data)[[df.name]])
          } else {
            var.names <- setdiff(all.vars(by_call$data), df.name)
          }
        }
      }
    }
  ...
  if (length(by_pos) == 1) {
    ...
    by.var <- deparse(by_call$INDICES)
    ...
    # by.first and by.last are used by print.summarytools when printing objects
    ...
    if (length(names(by_call$data)) == 1) {
      ...
      var.names <- c(deparse(by_call$data$x), deparse(by_call$data$y)) 
    }
  }
  # End of "by"
  ...
  # No by() but rather lapply() ------------------------------------------------
    ...
    df.name <- setdiff(all.names(lapply_call$X), c("[", "[[", "$"))[1]
    ...
    for (i_ in seq_along(sys_frames)) {
      if (identical(names(sys_frames[[i_]])[1:3], c("i", "X", "FUN"))) {
        var.names  <- names(df_[sys_frames[[i_]]$i])
        var.labels <- label(df_[sys_frames[[i_]]$i])
        break
      }
    }
  }
  ...
  # From here code applies no matter what if df.name and/or var.names are
  # not found yet --------------------------------------------------------------
  
  # Look for the data frame name (df.name)
  if (length(df.name) == 0) {
    if (exists("by_call")) {
      data.str <- deparse(by_call$data)
      allnames <- all.vars(by_call$data)
    } else {
      data.str <- deparse(match_call[[var]])
      ...
      for (obj.name in allnames) {
        if (exists(obj.name, envir = sys_frames[[no.frame]], mode = "list")) {
          ...
          if (is.data.frame(df_)) {
            ...
            if (max.varnames > 0 && length(var.names) == 0) {
              ...
              if (length(intersect(allnames, col_names)) > 0) {
                ...
                if (!grepl("[!><=()]", check_expr)) {
                  var.names <- intersect(allnames, col_names)[1]
                  var.labels <- label(df_[var.names])
                } else {
                  var.names <- sub("^(.+?)\\[.+$", "\\1", check_expr)
                  var.names <- sub(paste0("^", df.name, "\\$"), "", var.names)
                  if (var.names %in% col_names) {
                    var.labels <- label(df_[[var.names]])
                  } else if (!exists("dnn", envir = parent.frame()) ||
                                     is.list(parent.frame()$dnn)) {
                    message("Variable name '", var.names, "' was guessed but ",
  ...
  # Found df.name but not var.names --------------------------------------------
  if (max.varnames > 0 && length(data.str) > 0) {
    #  && length(setdiff(df.name, allnames)) > 0) {
    if (length(df.name) == 1 && length(var.names) == 0) {
      ...
      if (grepl(re_singlBrackets, data.str, perl = TRUE)) {
        # Isolate column indexing
        col_indexing <- 
          trimws(sub(re_singlBrackets, "\\4", 
                     x = data.str, perl = TRUE))
        
        # Try to extract var.names based on numeric indexing (if indexing
        ...
        if (grepl("\\d+", col_indexing)) {
          ...
          if (col_num <= ncol(df_)) {
            var.names  <- colnames(df_[col_num])
            var.labels <- label(df_[col_num])
          }
          
        } else if (grepl("\\d+:\\d+", col_indexing)) {
          col_nums <- do.call(":", as.list(unlist(strsplit("1:3",":"))))
          if (all(col_nums <= ncol(df_))) {
            var.names <- names(df_[col_nums])
            var.labels <- label(df_[col_nums])
          }
        }
      } else if (grepl(re_dblBrackets, data.str, perl = TRUE)) {
    ...
    if (length(df.name) ==1 && length(var.names) == 0) {
      ...
      potential.iterator <- setdiff(allnames, df.name)
        ...
        for (no.frame in seq_along(sys_frames)) {
          ...
          for (iter in potential.iterator) {
            if (exists(iter, envir = sys_frames[[no.frame]], mode = "numeric")) {
              ...
              var.names <- head(names(df_[it]), max.varnames)
              var.labels <- label(df_[it])[1]
              cont <- FALSE
              break
            }
          }
  ...
  if (length(df.name) == 0 && length(var.names) == 0) {
    cont <- TRUE
      ...
      for (no.frame in seq_along(sys_frames)) {
        ...
        for (obj.name in allnames) {
          if (exists(obj.name, envir = sys_frames[[no.frame]], 
                     mode = "numeric")) {
            var.names <- obj.name
            var.labels <- label(get(obj.name, envir = sys_frames[[no.frame]]))
            cont <- FALSE
            break
          }
        }
  ...
  # in a logical manner
  if (length(var.names) == 0 && length(df.name) == 0) {
    if (length(allnames) == 1) {
      ...
      if (grepl("freq|descr|ctable")) {
        df.name <- allnames[1]
        var.names <- allnames[2]
      }
    }
  }
  
  if (length(var.names) > max.varnames) {
    length(var.names) <- max.varnames
  }
  ...
  # Remove dataframe name from by.group if df.name is present in the 
  # by.group string and the same
  if (length(by.group) == 1 && length(df.name) > 0) {
    by.group <- sub(pattern = paste0(df.name,"$"), replacement = "", 
                    x = by.group, fixed = TRUE)
    var.names <- sub(paste0("^", df.name, "\\$"), "", var.names)
  }
  
  output <- list(df.name    = df.name,
                 df.label   = df.label,
                 var.names  = var.names,

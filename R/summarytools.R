...
desc <- function(x, na.rm=TRUE, style="simple", round.digits=2,
...
print.summarytools <- function(x, method="pander", ...) {
  ...
  # Create a string containing data info
  txt.conn <- textConnection(object = "info.table", open = "w")
  sink(txt.conn)
  for(a in c("var.name", "df.name", "var.label", "rows.subset", "date")) {
    if(a %in% names(attributes(x)))
      cat(a, ":", as.character(attr(x, a)),"\n\n",sep = "")
  }
  sink()
  close(txt.conn)

  info.table <- sub("df.name:",    "\nDataframe name: ", info.table)
  info.table <- sub("var.name:",   "\n Variable name: ", info.table)
  info.table <- sub("var.label:",  "\nVariable label: ", info.table)
  info.table <- sub("rows.subset:","\n   Rows subset: ", info.table)
  info.table <- sub("date:",       "\n          Date: ", info.table)


  notes <- ifelse("notes" %in% names(attributes(x)), yes = paste("Notes:", attr(x,"notes")), no = "")

  html.footer.line = "Generated with <a href='https://github.com/dcomtois/summarytools'>summarytools</a> package with basic <a href='http://getbootstrap.com'>Bootstrap</a> css."
      ...
      html.content <- tags$html(
        ...
        tags$body(
          div(class="container", style="width:80%",
              ...
              p(info.table),
              br(),
              HTML(gsub("<td> ", "<td>", desc.table.html)),
              h3("Observations"),
  ...
  # Objet de type 'dfSummary'
    ...
    if(method=="pander") {
      ...
      html.content <- tags$html(
        ...
        tags$body(
          div(class="container", style="width:80%",
              h1("Dataframe Summary"),
              h2(paste(attr(x, "df.name"), "(N = ", attr(x, "n.obs"), ")",sep="")),
              p(attr(x, "date")),
              #p("N = ", attr(x, "n.obs")), #class="text-center"),
              br(),
...
frequencies <- freq # for backward-compatibility
unistats <- desc # for backward-compatibility

...
txthist <- function(data) {
  ...
  for (ro in seq_len(nrow(graph))) {
    graphlines[ro] <-  strip(paste(graph[ro,], collapse = ""), "right")
  }
...
crunch_factor <- function() {
  ...
  if (parent.frame()$n_levels == 0 && parent.frame()$n_valid == 0) {
    ...
    outlist[[1]] <- paste0(1:n_levels,"\\. ", levels(parent.frame()$column_data), collapse = "\\\n")
    outlist[[2]] <- "All NA's"
    outlist[[3]] <- ""
    outlist[[4]] <- ""
    
  } else if (n_levels <= parent.frame()$max.distinct.values) {
    outlist[[1]] <- paste0(1:n_levels,"\\. ", 
                          substr(levels(parent.frame()$column_data), 1, 
                                 parent.frame()$max.string.width),
                          collapse = "\\\n")
    counts_props <- align_numbers_dfs(counts, round(props, parent.frame()$round.digits + 2))
    outlist[[2]] <- paste0("\\", counts_props, collapse = "\\\n")
    if (isTRUE(parent.frame()$graph.col) && any(!is.na(parent.frame()$column_data))) {
      outlist[[3]] <- encode_graph(counts, "barplot", parent.frame()$graph.magnif)
    ...
    n_extra_levels <- n_levels - parent.frame()$max.distinct.values
    
    outlist[[1]] <- 
      paste0(1:parent.frame()$max.distinct.values,"\\. ",
             substr(levels(parent.frame()$column_data), 1, 
                    parent.frame()$max.string.width)[1:parent.frame()$max.distinct.values],
             collapse="\\\n")
    ...
    counts_props <- align_numbers_dfs(
      c(counts[1:parent.frame()$max.distinct.values],
        sum(counts[(parent.frame()$max.distinct.values + 1):length(counts)])),
      c(props[1:parent.frame()$max.distinct.values],
        round(sum(props[(parent.frame()$max.distinct.values + 1):length(props)]),
              parent.frame()$round.digits + 2))
    )
    
    outlist[[2]] <- paste0("\\", counts_props, collapse = "\\\n")
    
    if (isTRUE(graph.col) && any(!is.na(parent.frame()$column_data))) {
      # prepare data for barplot
      tmp_data <- parent.frame()$column_data
      levels(tmp_data)[parent.frame()$max.distinct.values + 1] <- 
        paste("[", n_extra_levels, "others", "]")
      tmp_data[which(as.numeric(tmp_data) > parent.frame()$max.distinct.values)] <-
        paste("[", n_extra_levels, "others", "]")
      levels(tmp_data)[(parent.frame()$max.distinct.values + 2):n_levels] <- NA
      outlist[[3]] <- encode_graph(table(tmp_data), "barplot", parent.frame()$graph.magnif)
...
crunch_character <- function() {
  ...
  if (isTRUE(parent.frame()$trim.strings)) {
    column_data <- trimstr(parent.frame()$column_data)
  }
  ...
  if (n_empty == parent.frame()$n_tot) {
    ...
    if (!isFALSE(barcode_type <- detect_barcode(trimmed <- 
                                                trimstr(parent.frame()$column_data)))) {
    ...
    } else if (length(counts) <= parent.frame()$max.distinct.values + 1) {
      
      # Report all frequencies when allowed by max.distinct.values
      outlist[[1]] <- paste0(1:length(counts), "\\. ",
                            substr(names(counts), 1, parent.frame()$max.string.width),
                            collapse="\\\n")
      counts_props <- align_numbers_dfs(counts, round(props, parent.frame()$round.digits + 2))
      outlist[[2]] <- paste0(counts_props, collapse = "\\\n")
      ...
      n_extra_values <- length(counts) - parent.frame()$max.distinct.values
      outlist[[1]] <- paste0(
        paste0(1:parent.frame()$max.distinct.values,"\\. ",
      ...
      counts_props <- align_numbers_dfs(
        counts = c(counts[1:parent.frame()$max.distinct.values],
                   sum(counts[(parent.frame()$max.distinct.values + 1):length(counts)])),
        props = c(props[1:parent.frame()$max.distinct.values],
                  round(sum(props[(parent.frame()$max.distinct.values + 1):length(props)]),
                        parent.frame()$round.digits + 2))
      )
      outlist[[2]] <- paste0(counts_props, collapse = "\\\n")
      ...
      if (isTRUE(parent.frame()$graph.col)) {
        ...
        counts[parent.frame()$max.distinct.values + 1] <-
          sum(counts[(parent.frame()$max.distinct.values + 1):length(counts)])
        names(counts)[parent.frame()$max.distinct.values + 1] <- 
          paste0("[ ", n_extra_values, " others ]")
        counts <- counts[1:(parent.frame()$max.distinct.values + 1)]
        outlist[[3]] <- encode_graph(counts, "barplot", parent.frame()$graph.magnif)
        outlist[[4]] <- txtbarplot(prop.table(counts))
      }
    }
...
crunch_numeric <- function() {
  ...
  if (parent.frame()$n_miss == parent.frame()$n_tot) {
    ...
    if (!isFALSE(barcode_type <- detect_barcode(parent.frame()$column_data))) {
      ...
      outlist[[1]] <- paste(
        "mean (sd) : ", round(mean(parent.frame()$column_data, na.rm = TRUE),
                              parent.frame()$round.digits),
        " (", round(sd(parent.frame()$column_data, na.rm = TRUE), 
                    parent.frame()$round.digits), ")\\\n",
        "min < med < max :\\\n", round(min(parent.frame()$column_data, na.rm = TRUE), 
                                       parent.frame()$round.digits),
      ...
      if (length(counts) == 2 && counts[1] != counts[2]) {
        ...
        outlist[[1]] <- 
          paste(outlist[[1]],
                "IQR (CV) : ", round(IQR(parent.frame()$column_data, na.rm = TRUE),
                                     parent.frame()$round.digits),
                " (", round(sd(parent.frame()$column_data, na.rm = TRUE) / 
                              mean(parent.frame()$column_data, na.rm = TRUE),
                            parent.frame()$round.digits), ")", collapse="", sep="")
      }
    }
    ...
    if (length(counts) <= parent.frame()$max.distinct.values &&
      ...
      outlist[[2]]  <- 
        paste(paste0(rounded_names <- format(round(as.numeric(names(counts)), 
                                                   parent.frame()$round.digits), 
                                             nsmall = parent.frame()$round.digits * 
      ...
      if (parent.frame()$n_miss == 0 &&
          (isTRUE(all.equal(parent.frame()$column_data, 
                            min(parent.frame()$column_data):max(parent.frame()$column_data))) ||
           isTRUE(all.equal(parent.frame()$column_data, 
                            max(parent.frame()$column_data):min(parent.frame()$column_data))))) {
    ...
    if (isTRUE(parent.frame()$graph.col)) {
      if (length(counts) <= parent.frame()$max.distinct.values) {
        ...
        outlist[[3]] <- encode_graph(parent.frame()$column_data, "histogram", parent.frame()$graph.magnif)
...
crunch_time_date <- function() {
  ...
  if (parent.frame()$n_miss == parent.frame()$n_tot) {
    ...
    # Report all frequencies when allowed by max.distinct.values
    if (length(counts) <= parent.frame()$max.distinct.values) {
      outlist[[1]] <- paste0(1:length(counts),". ", names(counts), collapse="\\\n")
      props <- round(prop.table(counts), parent.frame()$round.digits + 2)
      counts_props <- align_numbers_dfs(counts, props)
      ...
      if (isTRUE(graph.col)) {
        tmp <- as.numeric(parent.frame()$column_data)[!is.na(parent.frame()$column_data)]
        outlist[[3]] <- encode_graph(tmp - mean(tmp), "histogram", parent.frame()$graph.magnif)
...
crunch_other <- function() {
  ...
  } else if (length(counts) <= parent.frame()$max.distinct.values) {
    props <- round(prop.table(counts), parent.frame()$round.digits + 2)
    counts_props <- align_numbers_dfs(counts, props)
    outlist[[2]] <- paste0(counts_props, collapse = "\\\n")
    
  } else {
    outlist[[2]] <- paste(as.character(length(unique(parent.frame()$column_data))), 
                          "distinct val.")
  }
  outlist[[3]] <- ""
  outlist[[4]] <- ""

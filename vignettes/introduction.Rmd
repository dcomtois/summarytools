...
is used to format the expression):
...
freq outputs, using the `recalculate` argument, which is TRUE by default.
...
```{r, results='markup'}
tobacco |> group_by(gender) |> freq(smoker) |> tb()
```
...
on how to proceed. There is a bit of tweaking to be done in order for the

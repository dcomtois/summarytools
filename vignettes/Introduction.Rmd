...
**summarytools** provides functions centered on data exploration and simple reporting. 
...
use it in 3 distinct ways:
  ...
  - To select rows by name, either use
    ...
    + a single character string; it will be used as a regular expression
      to select all the matching column(s)
...
## 3.3 Chi-Square ($\chi$^2^) Statistic
...
Results can be transposed by using `transpose = TRUE`. Desired statistics
...
Other notable features for this function:
 ...
 - Detects email addresses and produces counts for valid,
...
and *NA* columns are redundant, we can drop either one of them.
...
found [further below](#tmp-img-dir). This parameter can be ignored when
using the *html* rendering feature.
...
# 7. Grouped Statistics Using dplyr::group_by()
...
# 14. Graphs in Markdown dfSummaries {#tmp-img-dir}

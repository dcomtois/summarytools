...
More information on this function canbe found  in the 
...
To account for the frequencies of undisplayed values, the "(Other)" row is
...
There is no direct way to create a *pdf* file with **summarytool**. One option is
...
The class definitions from *summarytools.css* are included by default whenver
*html* rendering is done (either with `view()` or with `print()` .................)


While retaining or not the default **Bootstrap** CSS styling, 
it is possible to use a custom *CSS* file with additionnal class
definitions. 
...
**There is a pre-defined class called `st-small`** which uses a 13px font-size
of 
...
It is also possible to programatically define one or several

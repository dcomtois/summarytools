...
functions are at the heart of the package:
...
the package can serve both as a data exploration _and_ reporting tool, which can
be used either on its own for minimal reports, or along with larger sets of
tools such as RStudio's for [rmarkdown](http://rmarkdown.rstudio.com/), and 
...
Notably, [radiant](https://cran.r-project.org/web/packages/radiant/index.html), a
Shiny-based package for business analytics uses `dfSummary()` to describe
...
[document/vignette](http://htmlpreview.github.io/?https://github.com/dcomtois/summarytools/blob/dev-current/doc/Introduction%20to%20summarytools.htm) gives
a detailed description of all the changes, but a summary can be found [at the end of
this document](#changes).

Now we'll describe the four main functions of the package.
...
proportions. Since this vignette was created using _markdown_, we set the 
`style` argument's value to "rmarkdown" to take advantage of it.
...
We can simplify further and omit the _Totals_ row by setting `totals = FALSE`.

To get familiar with the output styles, try different values for `style` and see
how results look in the console. 
...
Since *markdown* has not full support (yet) for multiline headings, we’ll
first show a rendered _html_ version of the output:

```{r, results = "markup"}
with(tobacco, print(ctable(smoker, diseased), method = 'render'))
```

Note that we have to set knitr chunk option `results` to "markup" for the
results to be rendered correctly.

When viewed in a browser or in RStudio's Viewer, the results look like this:

```{r, eval=FALSE}
with(tobacco, view(ctable(smoker, diseased)))
```
<img src="ctable-with-row-props.png" style="border:0" />
...
instead of their ascii replacements. <!-- TODO! -->
...
To see an _html_ version of these results, we'd simply use `view()` (also possible
is to use `print()` with the argument `method = "viewer"`):
...
A **special situation** occurs when we want grouped statistics **for one variable only** 
using `descr()`. Instead of showing several tables having only one column 
(or row if we use transposition) each, **summarytools** will assemble everything 
into a single table:
...
more on _knitr_'s options.
  ...
  - To get better results when generating _html_ output with `method = 'render'`,
    set up your .Rmd document so it includes **summarytools**’ css.
...
# library(summarytools)  
# 
# print(dfSummary(tobacco, style = 'grid', plain.ascii = FALSE, graph.magnif = 0.78), 
...
Note that the original attributes which we changed temporarily for printing with these overrides
are still part of the _age\_stats_ object.
...
.. and then, to apply it to a **summarytools** object and display it in your browser:
...
To include **summarytools** functions into _shiny_ apps, it is recommended that you:

 - set `bootstrap.css = FASE` to avoid interacting with the app's layout  
 - omit headings by setting `headings = FALSE` in the call to `print()` or `view()`
 - adjust the size of the graphs in `dfSummary()` using the `graph.magnif` parameter
 - in case of problems with `dfSummary()` outputs, try omitting a column or two
   and setting the column widths with the `col.widths` parameter of the `print()`
   method
...
Versions 0.9.0 and 0.8.9 (which was only released on GitHub) brought many
improvements to **summarytools**. This [document-vignette](https://TODO) gives
a detailed view on those changes, but here are the main elements:
 ...
 - Translations
   ...
   + An additional function, `useTranslations()` allows using a custom translations
     file; a _csv_ template is available 
 ...
 - Improved printing of list objects
   + Objects of class "by" are now printed in the console with optimal results;
     no more need for `view(x, method = "pander"); this includes automatic
     printing (when the results are not assigned to a variable, or when we just
     type in the variable name in the console and press _Enter_
 ...
 - Easier management of global settings with `st_options()` 
   +  The `st_options()` function now has as many parameters as there are options
      to set, making it possible to set all options (if ever that is necessary)
 ...
 - Changes to `descr()`
   + For the `stats` argument, Values "fivenum" and "common" are now allowed,
     the latter representing the collection of _mean_, _sd_, _min_, _med_, _max_,
     _n.valid_, and _pct.valid_
  + Improved outputs when using `by()` 

 - Changes to `freq()`
   + As mentioned earlier, the function now accepts data frames as main argument;
     this makes obsolete the use of `lapply()` with it, and simplifies the
     subsequent printing, as the lists returned by `freq()` will also have the
     class **summarytools**, ensuring their proper dispatch to the right `print()`
    method.
  + Improved outputs when using `by()`
...
** Other notable changes **
 ...
 - Because it was subject to errors, the _Rows Subset_ heading element
   ...
   future release (just let me known by email or on GitHub if this is what you think)
...
## Limitations
  ...
  - The text histograms in dfSummary are not shown when using `rmarkdown`. Better use the
    'render' method.
  - It is not possible to control the heading levels of individual items. You can however 
    choose to omit the headings altogether with `headers = FALSE`.

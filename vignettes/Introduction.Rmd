...
## 1.3 Other Characteristics
 ...
 - **Flexible and extensible**: 
   + The built-in features used to support alternate languages provide a way to
     modify a great number of terms used in ouputs (headings and tables) 
   + **Pipe operators** from
...
Thankfully, `st_options()` is there to accomodate everyone's preferences (see
...
is not required since `freq()` accpts data frames as the main argument:
...
:::infobox
Using pipes makes it easier to generate `ctable()` results.
:::
...
argument. The default value is “all”, and it can be modified using
`st_options()`:
...
`warning` chunk option: `{r chunk_name, results="asis", warning=FALSE}`.
...
## 15.3 Defining Selected Keywords

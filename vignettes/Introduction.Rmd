...
<a href="#s-4">&#171; 4. Grouped Statistics : group_by()</a> | 
<a href="#header">< ToC ></a> | 
<a href="#s-6">6. Directing Output to Files &#187;</a><br/>  
</div><a id="s-5"></a>  
...
## 12.2 Defining and Using Customized Terms
...
instance, you might prefer using "N" over "Freq" in the title row of `freq()`
tables. Or you might need to generate a document with several `dfSummary()` 
tables, each having their own title.

For this, we can use `define_keywords()` to replacing the generic "Data Frame
Summary" with whatever is needed.
...
We can further customize the headings by adding arguments to the `print()`
function. Here, we use an empty string to override the value of `Variable`:

```{r}
define_keywords(title.freq = "Types and Counts, Iris Flowers")
define_keywords(freq = "N")   # "Freq" becomes "N" in table heading
print(
  freq(iris$Species,
...
By using the empty string, we caused the line normally used for printing the
variable name to disappear altogether (as opposed to being left blank).

See `?define_keywords` for a list of all customizable terms in the package.

Calling `define_keywords()` without any arguments will bring up, on systems that 
support graphical devices (the vast majority, that is), a window from which 
we can edit all the terms we want. 
...
"custom language" to a *csv* file that you can later use by calling
...
<a href="#header">< ToC ></a> | 
<a href="#s-14">14. Conclusion &#187;</a><br/>  
</div><a id="s-13"></a>  
<a id="setup"></a>  

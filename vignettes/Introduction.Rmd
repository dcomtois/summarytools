...
Version 0.9 brings **many** changes and improvements to **summarytools**. A
summary of those changes can be found [at the end of this vignette](#changes).

<!-- ### This Vignette's Setup -->

<!-- To avoid redundancy in the code, some global summarytools options have been set. -->
<!-- See [this section](#st_rmd) for more details; the settings in use are very -->
<!-- similar to the ones given in the example. -->
...
```{r}
freq(iris$Species, style = "rmarkdown", report.nas = FALSE, headings = FALSE)
```
...
<!-- Since the `style` option has been globally set to "rmarkdown" in the vignette's -->
<!-- setup, we will omit it from future examples. -->
...
core summarytools functions. It produces a list-type object
containing as many elements as there are categories in the grouping variable.

**But why not just use `by()`?** The reason is that `by()` produces objects of
class "by()", which have a dedicated `print()` method which conflict with
...
function that is essentially a _clone_ of `by()`, except that the objects it
produces have the class "stby".
...
## Using summarytools in Rmarkdown documents {#st_rmd}
...
# opts_chunk$set(comment = NA, prompt = FALSE, cache = FALSE, results = 'asis')
# library(summarytools)
# st_options(plain.ascii = FALSE,   # This should figure in all Rmd documents
#            style = "rmarkdown",   # This too
#            dfSummary.varnumbers = FALSE,  # This will make summaries narrower
#            dfSummary.valid.col = FALSE,   # This too
#            subtitle.emphasis = FALSE   # Depending on the theme used, this
#                                        # setting can improve the headings layout
#            )
#```
#
# ```{r, echo=FALSE}
...
```{r}
print(dfSummary(tobacco), max.tbl.height = 300, method = "render")
```
...
print(age_stats, report.nas = FALSE, totals = FALSE, 
...
<img style="float: center;" src="freq-fr.png" width=320px />

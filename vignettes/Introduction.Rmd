...
Here's what the setup chunk looks like (further explanations are given further
below):
...
however, since we have set these options globally, they are not really needed.
For this reason, we will not include them from hereon.
...
Another way to do it would be using `lapply()`. However, automatic printing
...
extremely wide columns. To delete those temporary images, we can use the
`cleartmp()` function (although the use of this function inside the _Rmd_
document will cause rendering to fail).
...
around the `print()` method, specifying `method = 'viewer'`. When used outside 
*RStudio*, this `method` falls back on “browser” and the report is shown in the
...
**Why not just use `by()`?** The reason is that `by()` produces objects of
...
provides good guidelines, but here are a few tips to get started:
 ...
 - Always set the `knitr` chunk option `results = 'asis'`. You can do
   this on a chunk-by-chunk basis, but it is easier if you just set it globally:
...
#
# ```{r}
# print(dfSummary(tobacco, style = 'grid', graph.magnif = 0.75), 
...
| bootstrap.css             |    TRUE   | Include Bootstrap 4 css in _html_ standalones |
...
| dfSummary.silent       |  FALSE   | Hide messages re: temporary files        |

...
The same can be achieved using `dplyr::group_by`, but to avoid a 
warning about implicit `NA`'s, we them explicit with
`forcats::fct_explicit_na()`. Here, we cannot use the `%$%` operator;
to preserve the grouping information, we need to feed `freq()` two 
variables: the grouping variable and the analysis variable. We do so
by using `dplyr::select()`, making the syntax a bit cumbersome; some
better solution might come in the future. 
```{r}
library(dplyr)
library(forcats)
tobacco %>% 
...
The `totals = FALSE` argument was added to stay in the spirit of "tidy
data". 

#### Using parameters to refine results
Here is an example that illustrates the use of parameters:
```{r}

...
[Recommendations for R Markdown](https://cran.r-project.org/web/packages/summarytools/vignettes/Recommendations-rmarkdown.html)  
...
cleaning, exploring, and simple reporting. At its core reside the following 
four functions:
...
txt <- data.frame(
  ...
  Description = 
    c("**Frequency Tables** featuring counts, proportions, cumulative statistics as well as missing data reporting",
      ...
      "Extensive **Data Frame Summaries** featuring type-specific information for all variables in a data frame: univariate statistics and/or frequency distributions, bar charts or histograms, as well as missing data counts and proportions. Very useful to quickly detect anomalies and identify trends at a glance")
)
...
who benefit from its current and *future features*. I regularly receive feature
requests, and when I receive donations, I set aside some time to work on them,
making summarytools even more relevant for data scientists, students
and researchers around the world. And no matter how small the amount is,
...
## 1.5 Latest Features (version 1.0.0) 
 ...
 - Heading elements are now omitted when corresponding defined keyword is set
   to " " (empty string), making headings even more flexible 
 - Improved functionality for customized terms and translations (see the
  [Intro vignette](https://cran.r-project.org/web/packages/summarytools/vignettes/Introduction.html) for details)  
 - `dfSummary()` tables are better aligned horizontally (categories >> counts >>
   charts)  
...
<sup>\(*\)</sup> *This is actually an image, since GitHub doesn't allow 
the _CSS_ necessary to make this table appear as it should. The same remark applies to `ctable()` results that follow.*
...
![dfSummary Output displayed in RStudio's viewer](img/dfSummary-in-RStudio-Viewer.png)
...
[**kableExtra**](https://CRAN.R-project.org/package=kableExtra) package, have a look at some of its vignettes:
...
`vignette("Recommendations-rmarkdown", package = "summarytools")` for the details
on how to proceed.
...
- If the argument is explicitly supplied in the function call, it will have
  precedence over any stored value for the parameter (stored values are the 
  ones that are written to the object's attributes when using a core function,
  or the default stored in the summarytools global options list). 

- If the core function and the print/view functions are called together and
  have conflicting values for a parameter, it is always the print/view 
  arguments that win.

- The stored defaults (modified with st_options() or left as they are when
  loading the package) will be applied if the parameter values cannot be found
...
Or you might need to generate a document with several `dfSummary()` tables,
with customized headings (replacing the generic "Data Frame Summary").
For this, use `define_keywords()`. 
```{r}
define_keywords(title.freq = "Types of Iris Flowers")
freq(iris$Species)
```

You could further customize the headings by adding arguments to the `print()`
function:
```{r}
...
By using the empty string, we ensure that in the output heading, the line normally 
used for printing the variable name will be entirely deleted (as opposed to left 
blank).

See `?define_keywords` for a list of all customizable strings in the package.

### Calling `define_keywords()` Without any Arguments

This will bring up, on systems that support graphical devices (the vast
majority, that is), an editable window allowing to modify only the desired
item(s). 

<img src="img/define-keywords.png" width=95% style="border:2px;border-color:grey" />

After closing the edit window, you will be able to save your new 
"custom language" to a *csv* file that you can later reuse by calling
`use_custom_lang()`.
...
If **summarytools** helps you get your work done, please consider making a
donation. In doing so, you'll help me feel useful, but more importantly
contribute to the package's development and help other people like you 
who benefit from its current and *future features*. I regularly receive feature
requests, and when I receive donations, I set aside some time to work on them,

...
Tables with heading spanning over 2 rows are not fully supported in markdown (yet), but the result is close to acceptable.
...
For best results, use _grid_ style or HTML rendering. Both have unresolved issues with the graphics column, although good progress has been made on that front since earlier versions.
...
The transparency of the graph images background is unfortunately lost in translation, but at least it is possible to see all graphs.

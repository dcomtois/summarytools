...
**Other notable changes:**   
 ...
 - The logical argument `omit.headings` is replaced by the more straightforward `headings` (still logical);
   the former is still supported but disappear in future releases 
 - The _Rows subset_ heading element has been removed, since it was subject to errors 
 - Under the hood, much has been going; the lengthy functions have been separated 
   into more manageable parts, facilitating maintenance and improving readability 
...
The following options are customizable with `st_options()`:
  ...
  **General options** 
  ...
  - `round.digits   = 2`
  - `plain.ascii    = TRUE`
  - `headings       = TRUE` (if using in a markdown document or a
     shiny app, setting this to `FALSE` might be preferable
  ...
  - `ctable.totals   = TRUE`
  - `descr.stats     = "all"` (other useful defaults are "fivenum" and "common")
